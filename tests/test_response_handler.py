...
from speaches.realtime.response_event_router import ResponseHandler, ResponseManager
...
async def test_create_and_run_response_sets_completed() -> None:
    ...
    completion_client = AsyncMock()
    completion_client.create = AsyncMock(return_value=_async_stream(chunks))

    ctx = MagicMock()
    ctx.completion_client = completion_client
