...
const SCENARIOS = ['A_full_drain_stt', 'B_barge_during_streaming_stt', 'D_cough_during_drain_stt'];
...
async function runOne(name) {
  ...
  if (name === 'D_cough_during_drain_stt') {
  ...
  const r = await lib.runPage(async (a) => {
    ...
    if (a.name === 'A_full_drain_stt') {
    ...
    } else if (a.name === 'B_barge_during_streaming_stt') {
      ...
      await silence(15000);
    } else if (a.name === 'D_cough_during_drain_stt') {
      log('sending question'); await send(a.question); await silence(700);
      const done = await waitFor(e => e.type === 'response.output_audio.done', 60000, 'audio.done');
      const ms = done.audio_duration_ms ?? 0;
...
lib.runAll(SCENARIOS, runOne, 'sessions_stt.json');
